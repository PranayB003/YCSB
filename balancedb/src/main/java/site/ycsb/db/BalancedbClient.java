...
public class BalancedbClient extends DB {
    ...
    public void init() throws DBException {
        ...
        ArrayList<String> ipAddressList = new ArrayList<>(Arrays.asList(ipAddressArray));

        // Append port to all IP addresses
        for (int i = 0; i < ipAddressList.size(); i++) {
            String ipAddress = ipAddressList.get(i);
            ipAddressList.set(i, ipAddress + ":" + port);
        }

        client = HttpClient(ipAddressList);
    }

    private static String mapToPayload(String key, Map<String, String> map) {
    ...
    private static void mapSubset(Map<String, String> map, Set<String> fields) {
